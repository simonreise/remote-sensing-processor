...
def s2postprocess(img, projection, cloud_mask, clip, normalize, path, path1, bandoutnames):
    ...
    if cloud_mask == True:
        ...
        if not isinstance(mask, type(None)):
            ...
            mask = xarray.where((xarray.apply_ufunc(rio.fill.fillnodata, mask, mask, dask = 'parallelized', keep_attrs = 'override', kwargs = {'max_search_distance': 5})) == 1, 0, 1) # fill 0 - nodata -> 1 - nodata, 0 - data
            # Emerge mask
