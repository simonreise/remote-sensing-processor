...
def load_model(model, bb, weights, input_shape, input_dims, num_classes, **kwargs):
    if model == 'BEiT':
        if weights != None:
            model = transformers.BeitForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.BeitConfig(image_size = input_shape, num_channels = input_dims, num_labels = num_classes, out_indices = [3, 5, 7, 11], **kwargs)
            model = transformers.BeitForSemanticSegmentation(config)
    elif model == 'ConditionalDETR':
        if weights != None:
            model = transformers.ConditionalDetrForSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.ConditionalDetrConfig(num_channels = input_dims, num_labels = num_classes, **kwargs)
            model = transformers.ConditionalDetrForSegmentation(config)        
    elif model == 'Data2Vec':
        if weights != None:
            model = transformers.Data2VecVisionForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.Data2VecVisionConfig(image_size = input_shape, num_channels = input_dims, num_labels = num_classes, out_indices = [3, 5, 7, 11], **kwargs)
            model =  transformers.Data2VecVisionForSemanticSegmentation(config)
    elif model == 'DETR':
        if weights != None:
            model = transformers.DetrForSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.DetrConfig(num_channels = input_dims, num_labels = num_classes, **kwargs)
            model = transformers.DetrForSegmentation(config)
    elif model == 'DPT':
        if weights != None:
            model = transformers.DPTForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.DPTConfig(image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
            model = transformers.DPTForSemanticSegmentation(config)
    elif model == 'Mask2Former':
        if weights != None:
            config = transformers.AutoConfig.from_pretrained(weights, num_labels = num_classes, **kwargs)
            ...
            config = transformers.Mask2FormerConfig(backbone_config = backbone, num_labels = num_classes, **kwargs)
        ...
        if weights != None:
            ...
            backbone = transformers.SwinConfig(image_size = input_shape, num_channels = input_dims, out_features = ["stage1", "stage2", "stage3", "stage4"])
            config = transformers.MaskFormerConfig(backbone_config = backbone, num_labels = num_classes, **kwargs)
            model = transformers.MaskFormerForInstanceSegmentation(config)
    elif model == 'MobileNetV2':
        if weights != None:
            model = transformers.MobileNetV2ForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.MobileNetV2Config(image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
            model = transformers.MobileNetV2ForSemanticSegmentation(config)
    elif model == 'MobileViT':
        if weights != None:
            model = transformers.MobileViTForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes  = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.MobileViTConfig(image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
            model = transformers.MobileViTForSemanticSegmentation(config)
    elif model == 'MobileViTV2':
        if weights != None:
            model = transformers.MobileViTV2ForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.MobileViTV2Config(image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        ...
        if weights != None:
            ...
            if bb == 'Swin' or bb == None:
                backbone = transformers.SwinConfig(image_size = input_shape, num_channels = input_dims, out_features = ["stage1", "stage2", "stage3", "stage4"])
                config = transformers.OneFormerConfig(backbone_config = backbone, num_labels = num_classes, **kwargs)
            elif bb == 'ConvNeXT':
        ...
        if weights != None:
            model = transformers.SegformerForSemanticSegmentation.from_pretrained(weights, ignore_mismatched_sizes = True, image_size = input_shape, num_channels = input_dims, num_labels = num_classes, **kwargs)
        else:
            config = transformers.SegformerConfig(num_channels = input_dims, num_labels = num_classes, **kwargs)
        ...
        if weights != None:
            ...
            if bb == 'ResNet':
                ...
                config = transformers.UperNetConfig(backbone_config = backbone, num_labels = num_classes, auxiliary_in_channels = 1024, **kwargs)
            elif bb == 'Swin' or bb == None:
                backbone = transformers.SwinConfig(image_size = input_shape, num_channels = input_dims, out_features = ["stage1", "stage2", "stage3", "stage4"])
                config = transformers.UperNetConfig(backbone_config = backbone, num_labels = num_classes, **kwargs)
            elif bb == 'ConvNeXT':
                backbone = transformers.ConvNextConfig(image_size = input_shape, num_channels = input_dims, out_features = ["stage1", "stage2", "stage3", "stage4"])
                config = transformers.UperNetConfig(backbone_config = backbone, num_labels = num_classes, **kwargs)
            elif bb == 'ConvNeXTV2':
        ...
        if weights != None:
            ...
            model = torchvision.models.segmentation.lraspp_mobilenet_v3_large(num_classes = num_classes, **kwargs)
        model.backbone['0'][0] = torch.nn.Conv2d(input_dims, 16, kernel_size = 3, stride = (2, 2), padding = (1, 1), bias=False)
    return model
    
    
def load_sklearn_model(model, bb, classification, epochs, **kwargs):
    if classification:
        if model == "Nearest Neighbors":
            ...
            if bb == 'lbfgs' or bb == None:
                ...
                clf = LogisticRegression(solver = 'newton-cg', n_jobs = -1, warm_start = True, verbose = 1, **kwargs)
            elif bb == 'newton-cholesky':
                clf = LogisticRegression(solver = 'newton-cholesky', n_jobs = -1, warm_start = True, verbose = 1, **kwargs)
            elif bb == 'sag':
                clf = LogisticRegression(solver = 'sag', n_jobs = -1, warm_start = True, verbose = 1, **kwargs)
            elif bb == 'saga':
                clf = LogisticRegression(solver = 'saga', penalty = 'elasticnet', l1_ratio = 0.5, n_jobs = -1, warm_start = True, verbose = 1, **kwargs)
        elif model == "SVM":
            if bb == 'rbf' or bb == None:
                ...
                clf = SVC(kernel = 'poly', probability = True, verbose = True, **kwargs)
            elif bb == 'sigmoid':
                clf = SVC(kernel = 'sigmoid', probability = True, verbose = True, **kwargs)
        elif model == "Gaussian Process":
            clf = GaussianProcessClassifier(n_jobs = -1, warm_start = True, **kwargs)
        elif model == "Decision Tree":
            clf = DecisionTreeClassifier(criterion = 'entropy', **kwargs)
        elif model == "Random Forest":
            clf = RandomForestClassifier(max_depth = 2, n_jobs = -1, warm_start=True, verbose = 1, **kwargs)
        elif model == "Gradient Boosting":
            clf = HistGradientBoostingClassifier(warm_start = True, verbose = 1000, **kwargs)
        elif model == "Multilayer Perceptron":
            clf = MLPClassifier(alpha = 1, max_iter = epochs, warm_start = True, verbose = True, **kwargs)
        elif model == "AdaBoost":
            ...
            clf = xgb.XGBClassifier(tree_method = "hist", verbosity = 3, n_jobs = -1, **kwargs)
        elif model == 'XGB Random Forest':
            clf = xgb.XGBRFClassifier(tree_method = "hist", verbosity = 3, n_jobs = -1, **kwargs)
    else:
        if model == "Nearest Neighbors":
            ...
            if bb == 'rbf' or bb == None:
                ...
                clf = SVR(kernel = 'poly', verbose = True, **kwargs)
            elif bb == 'sigmoid':
                clf = SVR(kernel = 'sigmoid', verbose = True, **kwargs)
        elif model == "Gaussian Process":
            clf = GaussianProcessRegressor(**kwargs)
        elif model == "Decision Tree":
            clf = DecisionTreeRegressor(**kwargs)
        elif model == "Random Forest":
            clf = RandomForestRegressor(max_depth = 2, n_jobs = -1, warm_start = True, verbose = 1, **kwargs)
        elif model == "Gradient Boosting":
            clf = HistGradientBoostingRegressor(warm_start = True, n_iter_no_change = 10, verbose = 1000, **kwargs)
        elif model == "Multilayer Perceptron":
            clf = MLPRegressor(alpha = 1, max_iter = epochs, warm_start = True, verbose = True, **kwargs)
        elif model == "AdaBoost":
            clf = AdaBoostRegressor(**kwargs)
        elif model == 'XGBoost':
            clf = xgb.XGBRegressor(tree_method = "hist", verbosity = 3, n_jobs = -1, **kwargs)
        elif model == 'XGB Random Forest':
            clf = xgb.XGBRFRegressor(tree_method = "hist", verbosity = 3, n_jobs = -1, **kwargs)
    model = clf
