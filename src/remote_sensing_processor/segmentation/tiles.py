...
def get_ss_tiles(x, y, tile_size, classification, shuffle, samples_file, split, x_outputs, y_outputs, x_dtype, y_dtype, x_nodata, y_nodata):
    ...
    for i in range(len(x)):
        with rio.open(x[i]) as bnd:  
            ...
            if bnd.nodata != x_nodata:
                warnings.warn(str(x[i]) + " nodata value is " + str(bnd.nodata) + ". It will be converted to " + str(x_nodata))
                container[:] = np.pad(np.where(bnd.read() == bnd.nodata, x_nodata, bnd.read()).astype(x_dtype), ((0,0),(0,shp_pad[0]),(0,shp_pad[1])), mode='constant', constant_values=0)
            else:
    ...
    if len(split) > 1:
        ...
        if x_outputs != None:
            with h5py.File(x_outputs[0], "w") as hf:
                dset = hf.create_dataset("data", data=x_tiles, compression="gzip", compression_opts=9, shuffle=False)
                dset.attrs['nodata'] = x_nodata
        x_bags.append(x_tiles)
        if y_outputs != None and y != None:
            with h5py.File(y_outputs[0], "w") as hf:
                dset = hf.create_dataset("data", data=y_tiles, compression="gzip", compression_opts=9, shuffle=False)
                dset.attrs['nodata'] = y_nodata
                dset.attrs['classification'] = classification
                dset.attrs['classes'] = classes
                dset.attrs['num_classes'] = num_classes
        if y != None:
            y_bags.append(y_tiles)
